...
## 高唐主站点开发规范（新需求 2026-04-15）

### 整体架构
- 站点管理分两级：**高唐主站点**（精细台账）+ **其他21个辅助站点**（轻量）
- 高唐站点 id = 's12'，在 Sites.tsx 列表中置顶、突出显示（加"主站点"标签）
- 点击高唐进入专属 GaotangDetail.tsx，其他站点保持现有 SiteDetail.tsx 不变
- App.tsx 新增路由 /sites/gaotang 指向 GaotangDetail.tsx
...
选择林分后，9个数据模块用 Tab 切换显示。

### 9个数据模块字段设计

**模块1：林地管理**
操作类型（灌溉/修枝/施肥/除草/打药/其他）、开始时间、结束时间、水表读数（仅灌溉）、操作人、备注

**模块2：土壤水分**
- 手动TRIME：测定日期、测定人、测定点位、坚果云路径、备注
- FDR自动：采集日期、采集人、数据时间范围、坚果云路径、备注
- 仪器维护：维护日期、操作人、维护类型（日常/故障）、仪器名、描述

**模块3：树干液流**
- 探针安装：安装日期、操作人、探针编号、方位(N/S/E/W)、安装高度(cm)、安装处直径(mm)、样树编号、胸径(cm)、树高(m)、冠幅(m)、枝下高(m)、备注
- 数据采集：采集日期、采集人、时间范围、坚果云路径
- 仪器维护：维护日期、操作人、维护类型、探针编号、描述

**模块4：树木生长**
- 每木检尺：测定日期、操作人、样树编号、胸径(cm)、树高(m)、枝下高(m)、冠幅(m)
- 茎干微生长传感器：同液流探针（安装/数采/维护）

**模块5：气象与地下水**
- 气象站采集：采集日期、采集人、时间范围、坚果云路径
- 气象站维护：维护日期、操作人、描述
- 手动地下水位：测定日期、测定人、测定点位、水位(m)、备注；折线图展示动态
- 自动气象展示（改进现有图表）：
  * 显示1个月数据（现在只有10天）
  * X轴日期字体加大确保可读
  * 修复重复日期bug：同一天只保留最新auto记录
  * 手动补充数据叠加显示，用不同颜色/形状标记，不覆盖auto数据

**模块6：实验设计**
处理数、重复数、处理编号、处理名称、具体措施、设计图片（可上传）、备注

**模块7：根系数据**
测定类型（微根管/土钻挖根）、测定日期、操作人、测定指标（根长密度/根生物量/根直径等）、坚果云路径、备注

**模块8：光合生理**
测定日期、操作人、测定仪器、测定指标（净光合/气孔导度/蒸腾/WUE等）、测定条件、坚果云路径、备注

**模块9：植物水力性状**
测定日期、操作人、器官类型（根系/枝条/叶片）、测定指标（导水率/P50/P88/TLP等）、测定方法、坚果云路径、备注

### 数据库新增表（Claude Code 需生成 SQL 脚本放在 scripts/ 目录）
- gaotang_stands（林分基本信息）
- gaotang_forest_mgmt（林地管理）
- gaotang_soil_moisture（土壤水分）
- gaotang_sapflow（树干液流）
- gaotang_tree_growth（树木生长）
- gaotang_meteo（气象与地下水）
- gaotang_experiment（实验设计）
- gaotang_roots（根系数据）
- gaotang_photosynthesis（光合生理）
- gaotang_hydraulics（水力性状）
所有表均需启用 RLS，admin 可读写，student 可读写自己提交的记录。

### 辅助站点改进（SiteDetail.tsx）
- 天气图表改为显示1个月，X轴字体加大
- 修复重复日期问题
- 新增简化版数据采集记录（谁、何时、采了什么）
- 新增仪器维护记录

### ✅ 高唐主站点任务完成（2026-04-15）
- [x] GT1. scripts/gaotang_schema.sql 生成（10张表，RLS，10个林分初始数据）——需手动在 Supabase SQL Editor 执行，见 TODO_MANUAL.md（commit 0c49146）
- [x] GT2. GaotangDetail.tsx 完成（10林分选择器 + 9数据模块 + 每模块列表+表单）（commit 25224f8）
- [x] GT3. Sites.tsx 高唐置顶+主站点标签+路由，App.tsx 新增 /sites/gaotang 路由（commit 25224f8）
