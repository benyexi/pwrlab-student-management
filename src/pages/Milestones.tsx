...
const STATUS_CONFIG: Record<MilestoneStatus, { color: string; icon: typeof CheckCircle }> = {
  '已完成': { color: 'bg-green-50 text-green-600', icon: CheckCircle },
  '进行中': { color: 'bg-blue-50 text-blue-600', icon: CircleDot },
  '未开始': { color: 'bg-gray-50 text-gray-500', icon: Clock },
  '已逾期': { color: 'bg-red-50 text-red-600', icon: AlertTriangle },
}
...
export default function Milestones() {
  ...
  const byStudent = useMemo(() => {
    ...
    // Sort milestones within each student by type order
